...
const getProductsByCategory = async (req, res) => {
  try {
    ...
    if (!category)
      return res.status(400).json({ success: false, products: [], message: "Category kerak" });
...
const addProduct = async (req, res) => {
  try {
    ...
    await product.save();
    res.status(201).json({ success: true, message: "Product muvaffaqiyatli qo'shildi!", product });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: "Server xatosi!" });
  }
...
const bulkAddProducts = async (req, res) => {
  try {
    ...
    if (!Array.isArray(products) || products.length === 0) {
      return res.status(400).json({ success: false, message: "Products array bo'lishi kerak!" });
    }

    const newProducts = await Product.insertMany(products);
    res.status(201).json({ success: true, message: "Products muvaffaqiyatli qo‘shildi!", products: newProducts });
  } catch (err) {
    console.error(err);
    res.status(500).json({ success: false, message: err.message });
  }
